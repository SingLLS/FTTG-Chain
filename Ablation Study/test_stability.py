...
"""
Stability Test for:
Vanilla Transformer + RaftGCN (Hybrid Ablation)

Evaluate 3 seeds and report:
...
class VanillaFTTRaft(nn.Module):
    ...
    def forward(self, x_norm, x_raw, topo):
        ...
        pred_ftt = self.ftt(x_norm)      # [B,2]
        feat_raft = self.raft(x_raw, topo)  # [B,64]
...
def load_dataset(csv_path, scaler_X):
    ...
    arrival = np.log1p(
        df["Actual Transaction Arrival Rate"].values.astype(float)
    )
    ...
    # same percentile clipping as training
    Y[:, 1] = np.clip(
        Y[:, 1],
        np.percentile(Y[:, 1], 1),
...
def main():
    ...
    model_paths = {
        "seed_42": "modelH/VT_RaftGAT_Ablation_42.pth",
        "seed_2024": "modelH/VT_RaftGAT_Ablation_2024.pth"
    }
